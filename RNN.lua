local RNN, parent = torch.class('prnn.RNN', 'nn.Module')
local ffi = require 'ffi'
local errcheck = prnn.errcheck
...
function RNN:__init(inputSize, hiddenSize, numLayers, batchFirst, dropout, rememberStates)
   ...
   self.datatype = 'PRNN_DATA_FLOAT'
   self.inputSize = inputSize
   self.hiddenSize = hiddenSize
   self.seqLength = 1
   self.miniBatch = 1
   self.numLayers = numLayers
   self.bidirectional = 'PRNN_UNIDIRECTIONAL'
   self.numDirections = 1 -- set to 2 for bi-directional.
   self.inputMode = 'PRNN_LINEAR_INPUT'
   self.mode = 'PRNN_RNN_RELU'
   ...
   self.backend = 'PRNN_PERSISTENT_BACKEND'
...
function RNN:createFilterDescriptors(count)
   return self:createDescriptors(count,
                            'prnnFilterDescriptor_t[?]',
                            'prnnCreateFilterDescriptor',
                            'prnnDestroyFilterDescriptor')
end

function RNN:createRNNDescriptors(count)
   return self:createDescriptors(count,
                            'prnnRNNDescriptor_t[?]',
                            'prnnCreateRNNDescriptor',
                            'prnnDestroyRNNDescriptor')
end

function RNN:createTensorDescriptors(count)
   return self:createDescriptors(count,
                            'prnnTensorDescriptor_t[?]',
                            'prnnCreateTensorDescriptor',
                            'prnnDestroyTensorDescriptor')
end
...
function RNN:resetWeightDescriptor()
   ...
   errcheck('prnnSetFilterNdDescriptor',
            self.wDesc[0],
            self.datatype,
            'PRNN_TENSOR_NCHW',
            3,
...
function RNN:resetIODescriptors()
   ...
   for i = 0, self.seqLength - 1 do
      ...
      errcheck('prnnSetTensorNdDescriptor',
               self.xDescs[i],
               self.datatype,
               3,
               dim:data(),
...
function RNN:updateOutput(input)
   ...
   assert(self.dropout == 0 or prnn.version >= 5103, 'dropout supported only in prnn v5.1 and above')
   ...
   self.workspace = prnn.getSharedWorkspace()
   local workspaceSize = torch.LongTensor(1)
   errcheck('prnnGetRNNWorkspaceSize',
            prnn.getHandle(),
            self.rnnDesc[0],
	    self.seqLength,
            self.xDescs,
   ...
   if self.train then
      ...
      errcheck('prnnGetRNNTrainingReserveSize',
               prnn.getHandle(),
               self.rnnDesc[0],
	       self.seqLength,
               self.xDescs,
      ...
      errcheck('prnnRNNForwardTraining',
               prnn.getHandle(),
               self.rnnDesc[0],
	       self.seqLength,
               self.xDescs, x:data(),
      ...
      errcheck('prnnRNNForwardInference',
               prnn.getHandle(),
               self.rnnDesc[0],
	       self.seqLength,
               self.xDescs, x:data(),
...
function RNN:updateGradInput(input, gradOutput)
   ...
   assert(self.dropout == 0 or prnn.version >= 5103, 'dropout supported only in prnn v 5.1 and above')
...
function RNN:accGradParameters(input, gradOutput, scale)
   ...
   assert(self.dropout == 0 or prnn.version >= 5103, 'dropout supported only in prnn 5.1 and above')
   assert(input:dim() == 3, 'input should have 3 dimensions: seqLength, miniBatch, inputSize')
   assert(input:size(1) == self.seqLength, 'input has incorrect sequence length!')
   assert(input:size(2) == self.miniBatch, 'input has incorrect minibatch size!')
   assert(input:size(3) == self.inputSize, 'input has incorrect size!')
   ...
   -- prnnRNNBackwardWeights doesn't accept a scale parameter so instead
   ...
   errcheck('prnnRNNBackwardWeights',
            prnn.getHandle(),
            self.rnnDesc[0],
	    self.seqLength,
            self.xDescs, x:data(),
...
local function numberOfLayers(self)
    if self.bidirectional == 'PRNN_BIDIRECTIONAL' then
...
local function retrieveLinearParams(self, cuDNNMethod)
    ...
    for layer = 0, numLayers - 1 do
        ...
        for layerId = 0, numberOfLinearLayers - 1 do
            ...
            local dataType = 'PRNN_DATA_FLOAT'
            local format = 'PRNN_TENSOR_NCHW'
            local nbDims = torch.IntTensor(1)

            local minDim = 3
            local filterDimA = torch.ones(minDim):int()
            errcheck('prnnGetFilterNdDescriptor',
                linLayerMatDesc[0],
                minDim,
                ffi.cast("prnnDataType_t*", dataType),
                ffi.cast("prnnDataType_t*", format),
                nbDims:data(),
                filterDimA:data())
...
function RNN:weights()
    return retrieveLinearParams(self, 'prnnGetRNNLinLayerMatrixParams')
end

function RNN:biases()
    return retrieveLinearParams(self, 'prnnGetRNNLinLayerBiasParams')
end
